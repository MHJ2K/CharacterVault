...
import kinkFetishTags from "./kink_fetish.json";
import metaTags from "./meta.json";
import sourceTags from "./source.json";
...
const FEMALE_IDENTITY_TAGS = ["female", "woman", "girl", "strong_woman", "grown_woman"] as const;

const FEMALE_ROLE_TAGS = ["mother", "daughter", "sister", "big_sister", "mommy_dom", "dommy_mommy", "milf", "gilf", "wife", "loving_wife", "girlfriend", "aunt", "stepmother", "stepsister", "muscle_mommy", "sugar_mommy", "widow"] as const;

const MALE_IDENTITY_TAGS = ["male", "boy", "man", "monster_boy", "incubus", "old_man"] as const;

const MALE_CROSSDRESSING_TAGS = ["femboy", "catboy", "twink", "trap", "sissy"] as const;

const MALE_ROLE_TAGS = ["father", "son", "brother", "big_brother", "daddy", "dilf", "husband", "boyfriend"] as const;
...
function buildTagExclusions(rules: readonly TagExclusionRule[]): Record<string, string[]> {
    ...
    return Object.fromEntries(Object.entries(map).map(([tag, exclusions]) => [tag, [...exclusions]]));
}

const TAG_EXCLUSIONS = buildTagExclusions(exclusionRules);

function deduplicateTagCategories(categories: readonly TagCategory[]): TagCategory[] {
...
export const TAG_CATEGORIES: readonly TagCategory[] = deduplicateTagCategories([
    ...
    { key: "kink_fetish", label: "Kink / Fetish", tags: kinkFetishTags },
    { key: "meta", label: "Meta", tags: metaTags },
    { key: "source", label: "Source", tags: sourceTags }
]);

let effectiveCategories: readonly TagCategory[] = TAG_CATEGORIES;

export function setEffectiveTagCategories(categories: readonly TagCategory[]): void {
    effectiveCategories = categories;
}

export async function loadEffectiveTagCategories(): Promise<readonly TagCategory[]> {
    const categories = await characterSettingsService.getStudioTagCategories();
    const normalized = categories.map((category) => ({ ...category, key: category.key as TagCategoryKey }));
    setEffectiveTagCategories(normalized);
    return effectiveCategories;
}

export function getEffectiveTagCategories(): readonly TagCategory[] {
    return effectiveCategories;
}

function getTagCategoryMap(): Record<string, string[]> {
    return Object.fromEntries(effectiveCategories.map(({ key, tags }) => [key, tags]));
}
...
export function getAllTags(): string[] {
    return Object.values(getTagCategoryMap()).flat() as string[];
}
...
export function buildConceptFromTags(selections: Record<string, string[]>): string {
    const parts: string[] = [];
    const orderedCategories = effectiveCategories.filter((category) => category.key !== "generation");

    for (const category of orderedCategories) {
        const tags = selections[category.key] ?? [];
 ...
 */
function drawTags(pool: readonly string[], min: number, max: number, excludedTags: Set<string>): string[] {
 ...
 */
export function randomizeTags(currentSelections: Record<string, string[]>, lockedKeys: readonly string[] = []): Record<string, string[]> {
    const next: Record<string, string[]> = { ...currentSelections };

    const availableKeys = new Set<string>(effectiveCategories.map((category) => category.key));
    const coreCategoryKeys = ["identity", "role", "personality"].filter((key) => availableKeys.has(key)) as unknown as TagCategoryKey[];
    const supportingKeys = ["genre", "appearance", "tone", "dynamic", "kink_fetish", "meta", "source"].filter((key) => availableKeys.has(key)) as unknown as TagCategoryKey[];
    ...
    for (const key of coreCategoryKeys) {
        ...
        next[key] = drawTags(getTagCategoryMap()[key] ?? [], 1, 2, excludedTags);
    ...
    for (const key of supportingKeys) {
        if (lockedKeys.includes(key)) continue;
        next[key] = drawTags(getTagCategoryMap()[key] ?? [], 0, 2, excludedTags);
        // Update exclusions after each selection
        excludedTags = getExcludedTags(next);
    }
